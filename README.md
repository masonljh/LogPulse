...
- **강력한 커스텀 로그 파싱**: Android Logcat은 물론 Nginx, Syslog 등 다양한 서버 및 시스템 로그를 지원합니다. 커스텀 정규식(Regex) 기반의 파싱 설정과 실시간 미리보기 다이얼로그를 통해 어떠한 형식의 로그라도 유연하게 분석할 수 있으며, 대용량 로그 파일(15만 줄 이상)도 매끄럽게 처리합니다.
